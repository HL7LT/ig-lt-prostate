// Canonical URLs for LT Prostate IG (profiles and terminology)
// Profiles: https://hl7.lt/fhir/prostate/StructureDefinition/{id}
// Terminology: https://tx.hl7.lt/fhir/ (no -lt-prostate in path)

// Profiles
Alias: $lesion-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/lesion-lt-prostate
Alias: $piqual-observation-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/piqual-observation-lt-prostate
Alias: $precise-assessment-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/precise-assessment-lt-prostate
Alias: $mpmri-report-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/mpmri-report-lt-prostate
Alias: $neoplasm-invasion-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/neoplasm-invasion-lt-prostate
Alias: $pelvic-organ-changes-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/pelvic-organ-changes-lt-prostate
Alias: $sequence-score-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/sequence-score-lt-prostate
Alias: $gleason-isup-observation-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/gleason-isup-observation-lt-prostate
Alias: $pirads-assessment-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/pirads-assessment-lt-prostate
Alias: $prostate-report-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/prostate-report-lt-prostate
Alias: $prostate-composition-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/prostate-composition-lt-prostate
Alias: $dre-observation-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/dre-observation-lt-prostate
Alias: $screening-careplan-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/screening-careplan-lt-prostate

// Terminology (tx.hl7.lt, no -lt-prostate in URL path)
Alias: $prostate-piqual-scale-url = https://tx.hl7.lt/fhir/CodeSystem/prostate-piqual
Alias: $prostate-piqual-vs-url = https://tx.hl7.lt/fhir/ValueSet/prostate-piqual
Alias: $prostate-precise-category-cs-url = https://tx.hl7.lt/fhir/CodeSystem/prostate-precise-category
Alias: $prostate-precise-category-vs-url = https://tx.hl7.lt/fhir/ValueSet/prostate-precise-category
Alias: $prostate-precise-change-pattern-cs-url = https://tx.hl7.lt/fhir/CodeSystem/prostate-precise-change-pattern
Alias: $prostate-precise-change-pattern-vs-url = https://tx.hl7.lt/fhir/ValueSet/prostate-precise-change-pattern
Alias: $prostate-precise-component-cs-url = https://tx.hl7.lt/fhir/CodeSystem/prostate-precise-component
Alias: $prostate-precise-component-vs-url = https://tx.hl7.lt/fhir/ValueSet/prostate-precise-component
Alias: $prostate-mri-type-url = https://tx.hl7.lt/fhir/ValueSet/prostate-mri-type
Alias: $prostate-damage-and-change-status-url = https://tx.hl7.lt/fhir/ValueSet/prostate-damage-and-change-status
Alias: $body-structure-prostate-url = https://tx.hl7.lt/fhir/ValueSet/body-structure-prostate
Alias: $prostate-sequence-type-url = https://tx.hl7.lt/fhir/ValueSet/prostate-sequence-type
Alias: $prostate-isup-grade-group-url = https://tx.hl7.lt/fhir/ValueSet/prostate-isup-grade-group
Alias: $prostate-pirads-sector-url = https://tx.hl7.lt/fhir/ValueSet/prostate-pirads-sector
Alias: $prostate-questionnaire-options-cs-url = https://tx.hl7.lt/fhir/CodeSystem/prostate-questionnaire-options
Alias: $prostate-questionnaire-options-cs = https://tx.hl7.lt/fhir/CodeSystem/prostate-questionnaire-options
Alias: $prostate-fhir-mapping-target-cs-url = https://tx.hl7.lt/fhir/CodeSystem/prostate-fhir-mapping-target
Alias: $prostate-adpp-questionnaire-item-cs-url = https://tx.hl7.lt/fhir/CodeSystem/prostate-adpp-questionnaire-item
Alias: $prostate-pathology-questionnaire-item-cs-url = https://tx.hl7.lt/fhir/CodeSystem/prostate-pathology-questionnaire-item
Alias: $snomed-prostate-extension-cs-url = https://tx.hl7.lt/fhir/CodeSystem/snomed-prostate-extension
Alias: $digital-rectal-examination-vs-url = https://tx.hl7.lt/fhir/ValueSet/digital-rectal-examination
Alias: $prostate-change-nature-vs-url = https://tx.hl7.lt/fhir/ValueSet/prostate-change-nature
Alias: $prostate-prior-damage-status-vs-url = https://tx.hl7.lt/fhir/ValueSet/prostate-prior-damage-status
Alias: $bladder-changes-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/bladder-changes-lt-prostate
Alias: $bone-metastasis-lt-prostate-url = https://hl7.lt/fhir/prostate/StructureDefinition/bone-metastasis-lt-prostate
