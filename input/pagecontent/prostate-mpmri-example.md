...
The example is based on a typical clinical mpMRI report containing PSA values, prostate volume, PI-RADS assessment, PRECISE follow-up, lesion localisation, and local staging findings (capsule contact, seminal vesicles, neurovascular bundles, lymph nodes, bladder and rectum).  
...
In this implementation guide, prostate MRI findings are modeled in a lesion-centric manner. Each relevant lesion is represented explicitly as a BodyStructure resource. Imaging-derived observations such as PI-RADS score and sequence-specific scores (T2, DWI, ADC, DCE) reference the lesion via the `focus` element. This allows multiple observations to be consistently linked to the same anatomical finding and supports longitudinal follow-up and comparison across imaging studies.
